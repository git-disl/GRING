...
func TestSendUnderMaxNumConnections(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		node, err := GRING.NewNode(GRING.WithNodeMaxOutboundConnections(1))
		assert.NoError(t, err)

		defer node.Close()

		node.Handle(func(ctx GRING.HandlerContext) error {
			wg.Done()
			return nil
		})
...
func TestSendWithTwoPeersUnderMaxNumConnections(t *testing.T) {
	...
	a, err := GRING.NewNode(GRING.WithNodeMaxOutboundConnections(1))
	assert.NoError(t, err)

	defer a.Close()

	a.Handle(func(ctx GRING.HandlerContext) error {
		wg.Done()
		return nil
	})
	...
	b.Handle(func(ctx GRING.HandlerContext) error {
		wg.Done()
		return nil
	})
...
func TestIdleTimeoutServerSide(t *testing.T) {
	...
	a, err := GRING.NewNode(GRING.WithNodeIdleTimeout(50 * time.Millisecond))
	assert.NoError(t, err)
...
func TestIdleTimeoutClientSide(t *testing.T) {
	...
	b, err := GRING.NewNode(GRING.WithNodeIdleTimeout(50 * time.Millisecond))
	assert.NoError(t, err)
...
func TestWithNodeMaxRecvMessageSize(t *testing.T) {
	...
	a, err := GRING.NewNode(GRING.WithNodeMaxRecvMessageSize(1 << 20))
	assert.NoError(t, err)
	defer a.Close()
