...
func (n *Node) Send(ctx context.Context, addr string, data []byte, opcode ...byte) error {
	...
	if(uint32(len(data)) >= n.maxRecvMessageSize-uint32(160)) {
	    var msize uint32
	    msize=0
	    length=0
	    for msize < uint32(len(data)) {
		length++
		msize += n.maxRecvMessageSize-uint32(160) // consider control field in a message: length(uint32) and nonce(uint64) recv_nonce(uint64)
	        //fmt.Printf("msize: %d , length : %d\n",msize, length)
	    }
		...
		if uint32(len(data)) - end < n.maxRecvMessageSize-uint32(160) {
		    end = uint32(len(data))
		}else {
		    end += (n.maxRecvMessageSize-uint32(160))
		}
	        ...
	        if err != nil {
                ...
                //fmt.Printf("send request nonce:%d, recv_nonce:%d, length:%d start:%d , end:%d\n",nonce,recv_nonce,i,start,end)
		if err := c.send(nonce, recv_nonce, data[start:end], i); err != nil {
		    ...
		    return err
	            ...
	            case msg = <-ch:
		        // TODO check ACK
                        //fmt.Printf("ACK received. nonce:%d recv_nonce:%d\n",nonce,msg.recv_nonce)
	    ...
	    //fmt.Printf("send message done\n")

	    return nil
	 }else { // single message
	    if err := c.send(0, math.MaxUint64, data, 0); err != nil { // single message length should be 0. nonce doesn't matter because we don't expect immediate resp.
		return err
	    }
	}
...
func (n *Node) Request(ctx context.Context, addr string, data []byte, opcode ...byte) ([]byte, error) {
	...
	msg, err := c.request(ctx, data, 0) //request should be length 0
	if err != nil {
		return nil, err
	}
