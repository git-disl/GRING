...
var (
	...
	max_peers=50
        ...
        //group_size=5 // group_size-1 = degree of tree = # of children
        group_size=7 // group_size-1 = degree of tree = # of children
	max_conn=1000
	discoverInterval = 60
...
func increaseNumBench(){
	mutex_client_ready.Lock()
        num_client_ready++
	fmt.Printf("call IncreaseClientReady: current: %d expected: %d\n",num_client_ready,overlay.GetTotMembers()+1)
	//if me is the last one. I report
	if num_client_ready >= (overlay.GetTotMembers()+1) {
            // reset the value after reporting
	    num := num_client_ready
	    ...
	    dummydata := make([]byte, datasize)
            usermsg := userMessage{Opcode: byte(OP_BENCH), Contents: dummydata}
            overlay.Report_GR(node.ID(), usermsg.Marshal(), num)
...
func on_fedcomp(p2pmsg dual.P2pMessage) {
	...
	ptr := unsafe.Pointer(&msg.Contents[0])
	    ...
	    if msg.Opcode == OP_BENCH {
	        time.Sleep(time.Duration(transfertime) * time.Second)

	        fmt.Printf("GO INIT : recv OP_BENCH\n")
                overlay.Multicast_GR(node.ID(), p2pmsg.Contents, dual.OP_FED_COMP)

                //go func() {
	        fmt.Printf("GO INIT : OP_BENCH start train\n")
		starttime = time.Now()
		// train my model
	        time.Sleep(time.Duration(mytraintime) * time.Second)
                duration := time.Since(starttime)
                fmt.Printf("GO INIT : OP_BENCH done train time:%s\n",duration)
	...
	if overlay.GetID() == dual.ID_SUBLEADER {
	    ...
	    if msg.Opcode == OP_BENCH {
	        ...
	        fmt.Printf("GO SUB : recv OP_BENCH\n")
                overlay.Multicast_GR(node.ID(), p2pmsg.Contents, dual.OP_FED_COMP)

		//go func() {
	        fmt.Printf("GO SUB : OP_BENCH start train\n")
		starttime = time.Now()
		// train my model
	        time.Sleep(time.Duration(mytraintime) * time.Second)
		duration := time.Since(starttime)
		fmt.Printf("GO SUB : OP_BENCH done train time:%s\n",duration)
	    ...
	    if msg.Opcode == OP_BENCH {
	        ...
	        fmt.Printf("GO WORKER : recv OP_BENCH\n")

		//go func() {
	        fmt.Printf("GO WORKER : OP_BENCH start train\n")
		starttime = time.Now()
		// train my model
	        time.Sleep(time.Duration(mytraintime) * time.Second)
		duration := time.Since(starttime)
		fmt.Printf("GO WORKER : OP_BENCH done train time:%s\n",duration)

                overlay.Report_GR(node.ID(), p2pmsg.Contents, 1)

                //dummydata := make([]byte, datasize)
                //usermsg := userMessage{Opcode: byte(OP_BENCH), Contents: dummydata}
                //overlay.Report_GR(node.ID(), usermsg.Marshal(), 1)
...
func on_report(p2pmsg dual.P2pMessage) {
	...
	if len(msg.Contents) != 0 {
        ...
        if overlay.GetID() == dual.ID_PUBLISHER {
            ...
            if msg.Opcode == OP_BENCH {
	        time.Sleep(time.Duration(transfertime) * time.Second)
	        fmt.Printf("GO PUB : recv OP_BENCH report\n")

                duration := time.Since(starttime)
                fmt.Printf("DONE Bench time:%s\n",duration)
	    }
        ...
        if overlay.GetID() == dual.ID_INITIATOR {
            ...
            if msg.Opcode == OP_BENCH {
	        time.Sleep(time.Duration(transfertime) * time.Second)
	        fmt.Printf("GO INIT : recv OP_BENCH report\n")

		mutex_client_ready.Lock()
                value := p2pmsg.Aggregation
	        //fmt.Printf("OP_REPORT value : %d\n",value)
	        num_client_ready += int(value)
                fmt.Printf("FL Server receives OP_BENCH REPORT num_client current: %d, total expected : %d\n",num_client_ready,  overlay.GetTotMembers()+1)
	        if num_client_ready >= overlay.GetTotMembers()+1 {
                    // reset the value after reporting
	            num := num_client_ready
		    num_client_ready = 0
		    mutex_client_ready.Unlock()
                    ...
                    overlay.Report_GR(node.ID(), p2pmsg.Contents, num)
                    fmt.Printf("GO INIT : Send report to publisher %s opcode: OP_REPORT value : %d\n",overlay.GetLeader(),num)
...
/*
					...
					toleranceRate -= toleranceReduceStep
				        ...
				        timerReportChecker.Reset(time.Duration(timerDurationReady) * time.Second)
	                                ...
	                                num := num_client_ready
                                        ...
                                        usermsg := userMessage{Opcode: byte(OP_BENCH), Contents: dummydata}
                                        overlay.Report_GR(node.ID(), usermsg.Marshal(), num)
                                        fmt.Printf("GO INIT : Send report to upper leader %s opcode: OP_REPORT value : %d\n",overlay.GetLeader(),num)
	                                fmt.Printf("GO INIT : timerReady is done\n")
		                        return
                                    }
	                    }//select
	...
	if overlay.GetID() == dual.ID_SUBLEADER {
            ...
            if msg.Opcode == OP_BENCH {
	        time.Sleep(time.Duration(transfertime) * time.Second)
                fmt.Printf("GO SUB : recv OP_BENCH report\n")

                mutex_client_ready.Lock()
                value := p2pmsg.Aggregation
                num_client_ready += int(value)
                fmt.Printf("GO SUB : Sub-leader receives OP_BENCH REPORT num_client_ready current: %d, total expected : %d\n",num_client_ready, overlay.GetTotMembers()+1)
                if num_client_ready >= (overlay.GetTotMembers()+1) {
                    // reset the value after reporting
		    num := num_client_ready
                    num_client_ready = 0
		    ...
		    toleranceRate = 100
					...
					toleranceRate -= toleranceReduceStep
				        ...
				        mutex_client_ready.Unlock()
	                                ...
	                                usermsg := userMessage{Opcode: OP_CLIENT_READY, Contents: nil}
				        overlay.Report_GR(node.ID(), usermsg.Marshal(), num)
	                                fmt.Printf("GO GUB : Send report to upper leader %s opcode: OP_REPORT value : %d\n",overlay.GetLeader(),num)
...
func init_p2p(host string, port int, serveraddr string){
	...
	node, err = GRING.NewNode(
		...
		GRING.WithNodeMaxRecvMessageSize(1<<22), //4MB
...
func bench(choose int) {
   ...
   err := node.SendMessage(ctx, string(initiator), dual.P2pMessage{Opcode: byte(dual.OP_FED_COMP), Contents: usermsg.Marshal() }, dual.OP_GROUP_MULTICAST)
...
func main() {
	...
	if err != nil {
	    ...
	    if len(args) == 5 {
                mytraintime, err = strconv.Atoi(args[3]) // for debug
                mynumleave, err = strconv.Atoi(args[4]) // for debug
	        fmt.Printf("mytraintime %d mynumleave %d\n",mytraintime, mynumleave)
	    }
	}else{
