...
type ops struct {
	...
	on_updateinitiator_after_regroup C.convert
...
var (
	...
	// For the initiator to store publishers subscriber list
	publisherSubList []dual.Subscriber
)
...
func Register_callback(name *C.char, fn C.convert) {
	...
	if C.GoString(name) == "on_update_initiator_after_regroup" {
		callbacks.on_updateinitiator_after_regroup = fn
	}
...
func GetPublisherSubList() []dual.Subscriber {
	return publisherSubList
}

func SetPublisherSubList(subs []dual.Subscriber) {
	publisherSubList = subs
}

// Save publishers subscriber list with inititator to facilitate re-structuring of tree
func SavePublisherSubscribers(msg dual.P2pMessage) {
	var pub_sub_array []dual.Subscriber
	sub_list := make(map[string]dual.Subscriber, 0)
	...
	for _, value := range sub_list {
		pub_sub_array = append(pub_sub_array, value)
	}
	SetPublisherSubList(pub_sub_array)
}

func SelectKPeers(peers []dual.Subscriber) []dual.Subscriber {
	if len(peers) == 0 {
		return peers
	}
	// Select k% peers which includes current initiator
	k := 0.5
	rand.Seed(time.Now().UnixNano())
	// Determine how many elements to select
	// -1 becuase we always have to select the initiator
	numToSelect := int(math.Ceil(k * float64(len(peers))))
	if numToSelect == 0 {
		numToSelect = 1
	}

	//fetch the index of initiator
	initiator_index := -1
	for i, val := range peers {
		if val.Key == node.ID().ID.String() {
			initiator_index = i
			break
		}
	}
	selectedIndicesSet := make(map[int]bool)
	selectedIndices := make([]int, numToSelect)
	selectedIndices[0] = initiator_index
	selectedIndicesSet[initiator_index] = true
	// Select K% (-1 initiator) peers
	for i := 1; i < numToSelect; {
		index := rand.Intn(len(peers))
		if !selectedIndicesSet[index] {
			selectedIndicesSet[index] = true
			selectedIndices[i] = index
			i++
		}
	}

	selectedArr := make([]dual.Subscriber, numToSelect)
	for i, index := range selectedIndices {
		selectedArr[i] = peers[index]
	}
	return selectedArr
}
...
// Create a new EdgeClientList.json file to print the nodes participating in a particular round
func create_edge_client_list_json(arr []dual.Subscriber, cur_round int) {
	...
	for i := 0; i < len(arr); i++ {
		if arr[i].Key == node.ID().ID.String() {
			initiator_address = arr[i].Addr
			continue
		}
		clients = append(clients, arr[i].Addr)
	}
...
func on_select_peers(msg dual.P2pMessage, cur_round int) []dual.Subscriber {
	SavePublisherSubscribers(msg)
	random_peers := SelectKPeers(GetPublisherSubList())
	create_edge_client_list_json(random_peers, cur_round)
	fmt.Println("Random peers * ", random_peers)
	return random_peers
}
...
func init_p2p(host string, port int, serveraddr string){
	...
	events = dual.Events{
		...
		*/
		...
		StartTrainingAfterRegroup: start_training_after_regroup,
	}
...
func start_training_after_regroup(model_metadata []byte) {
	ptr := unsafe.Pointer(&model_metadata[0])
	C.call_c_func(callbacks.on_updateinitiator_after_regroup, (*C.char)(ptr))
}
...
func SelectRandomPeer(peers []dual.Subscriber) dual.Subscriber {
	// id := node.ID().ID.String()
	// idx := 0
	// for id == node.ID().ID.String() {
	// 	idx = rand.Intn(len(peers))
	// 	id = peers[idx].Key
	// 	fmt.Println("Idx: ", idx, peers[idx].Addr)
	// 	fmt.Println(id, node.ID().ID.String())
	// }
...
//export Regroup
func Regroup(src *C.char, metadata_size C.int, next_round int) {
	model_metadata := C.GoBytes(unsafe.Pointer(src), C.int(metadata_size))
	peers := GetPublisherSubList()
	new_initiator := SelectRandomPeer(peers)
	pubSubList := make(map[string]dual.Subscriber)
	for _, key := range peers {
		pubSubList[key.Key] = key
	}
	Data := RegroupData{Peers: pubSubList, Publisher: overlay.GetLeader(), ModelMetadata: model_metadata, NextRound: next_round}
	data, _ := json.Marshal(Data)

	size := group_size

	ctx, cancel := context.WithCancel(context.Background())
	// send sub_list and local model to initiator
	msg := dual.P2pMessage{Aggregation: uint32(size), Opcode: byte(dual.OP_REQUEST_REGROUP), Contents: data}
	send_to := string(new_initiator.Addr)
	err := node.SendMessage(ctx, string(send_to), msg, dual.OP_GROUP_MULTICAST)
